...
export function initDatabase() {
  ...
  const dbPath = process.env.VERCEL 
    ? join(tmpdir(), 'foods-connect.db')
    : existsSync('/data')
    ? '/data/foods-connect.db'
    : join(process.cwd(), 'foods-connect.db');
  ...
  if (adminCount.count === 0) {
    ...
    console.log('   Username: 株式会社フーズコネクト');
    ...
    const hashedPassword = bcrypt.hashSync('admin123', 10);
    db.prepare('UPDATE administrators SET username = ?, password = ?, email = ? WHERE id = 1').run('株式会社フーズコネクト', hashedPassword, 'issui0707@gmail.com');
